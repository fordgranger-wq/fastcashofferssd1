export default function App() {
  ...
  const featureCards = [
    ...
    ["Faster Relief", "Potential access to funds during escrow for qualified sellers."]
  ];
    ...
    <div className="min-h-screen bg-[#f7f5ef] text-slate-900">
      <section className="px-6 py-16 lg:py-20">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
          <div>
            <div className="mb-4 inline-block rounded-full border border-[#8dd4cf] bg-white/80 px-4 py-2 text-sm font-semibold text-[#0d4f5c] shadow-sm">
              Multiple Cash Offers, One Simple Process
            </div>

            <h1 className="max-w-3xl text-4xl font-bold tracking-tight text-[#0f3d47] sm:text-5xl lg:text-6xl">
              Get Multiple Cash Offers On Your San Diego Home In 72 Hours
            </h1>

            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#27545f]">
              Compare serious offers from actual investors, not wholesalers, so you can sell as-is, move on your timeline, and protect your equity.
            </p>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {benefits.map((item) => (
                <div
                  key={item}
                  className="rounded-2xl border border-white/60 bg-white px-4 py-3 text-sm font-medium text-[#0d4f5c] shadow"
                >
                  {item}
                </div>
              ))}
            ...
            <div className="mt-8">
              <a
                href="#offer-form"
                className="inline-block rounded-2xl bg-[#f47a00] px-6 py-4 text-base font-semibold text-white shadow transition hover:opacity-90"
              >
                Get My Cash Offers
              </a>
            </div>
            ...
            <p className="mt-4 text-sm text-[#3d6770]">
              No obligation. No open houses. No agent commission pitch. Just a straightforward cash offer option for homeowners who want clarity.
            </p>
          </div>
          ...
          <div>
            <div
              id="offer-form"
              className="rounded-[2rem] border border-white/70 bg-white p-6 shadow-lg lg:p-8"
            >
              <div className="mb-6">
                ...
                <p className="mt-2 text-sm leading-6 text-[#38616a]">
                  Fill out the short form below and we’ll begin lining up qualified investor interest.
                </p>
              </div>
              ...
              <form className="space-y-4">
                ...
                <button
                  type="button"
                  className="w-full rounded-2xl bg-[#f47a00] px-6 py-4 text-base font-semibold text-white shadow transition hover:opacity-90"
                >
                  Get My Multiple Cash Offers
                </button>

                <p className="text-xs leading-5 text-[#547880]">
                  By submitting, you agree to be contacted about your property. Any advance during escrow is subject to transaction terms and qualification.
                </p>
              </form>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-6 lg:grid-cols-4">
            {featureCards.map(([title, text]) => (
              <div
                key={title}
                className="rounded-[1.75rem] border border-white/70 bg-white p-6 shadow"
              >
                <h3 className="text-lg font-semibold text-[#0f3d47]">{title}</h3>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 text-lg leading-8 text-[#ddf3f1]">
              Not everyone has the budget or the time to fix up their house to sell traditionally, and sometimes a cash offer is the best option. But why accept a low-ball offer when you can compare multiple cash offers and maximize your equity? Don't leave money on the table!
            </p>
          </div>
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-4">
            {steps.map((step, index) => (
              ...
              >
                ...
                <p className="mt-3 text-sm leading-6 text-[#ddf3f1]">{step.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-6 py-20">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[2rem] border border-white/60 bg-white p-8 shadow">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[#0d4f5c]/70">
              Why Use Fast Cash Offers SD?
            </p>
            <h2 className="mt-3 text-3xl font-bold text-[#0f3d47] sm:text-4xl">
              We're Not Just Offering Speed. We Are Offering Options. And Peace Of Mind.
            </h2>
            <div className="mt-8 space-y-6 text-[#3e676f]">
              <p className="text-lg leading-8">
                Most “cash offer” sites feel like a single low-ball offer in disguise. Or they are just selling your info to a bunch of investors and wholesalers who never stop calling. We are different. We are a single contact to dozens of vetted local investors in San Diego County. And they compete to buy your home.
              </p>
              <p className="text-lg leading-8">
                Whether you have an inherited home you need to sell, a rental property with deferred maintenance, retiring and relocating, or you simply want a clean exit without fixing everything first, this might be a great option.
              </p>
              <p className="text-lg leading-8">
                And if you don't like any of the offers you receive, you are not obligated to accept any of them.
              </p>
            </div>
          </div>

          <div className="rounded-[2rem] border border-white/65 bg-white p-8 shadow">
            <h3 className="text-2xl font-bold text-[#0f3d47]">Ideal Seller Situations</h3>
          ...
          <div className="mt-12 space-y-4">
            {faqs.map((item) => (
              <div
                ...
                className="rounded-[1.75rem] border border-white/70 bg-white p-6 shadow"
              >
                <h3 className="text-lg font-semibold text-[#0f3d47]">{item.q}</h3>
                <p className="mt-3 text-sm leading-7 text-[#406870]">{item.a}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-6 py-20">
        <div className="mx-auto max-w-7xl">
          <div className="rounded-[2.25rem] bg-[#0d4f5c] px-8 py-12 text-white shadow sm:px-12">
            <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
              <div>
                ...
                <p className="mt-4 max-w-2xl text-lg leading-8 text-[#ddf3f1]">
                  Whether the property is clean, cluttered, inherited, tenant-occupied, outdated, or distressed, you can start with a quick conversation and see what the market gives you.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-4">
                <a
                  href="#offer-form"
                  className="rounded-2xl bg-[#f47a00] px-6 py-4 text-center text-base font-semibold text-white shadow transition hover:opacity-90"
                >
                  Get My Cash Offers
                </a>
                <div className="rounded-2xl border border-white/15 bg-white/5 px-5 py-4 text-sm leading-6 text-[#ddf3f1]">
                  San Diego-focused • Multiple offers in 72 hours • Close in 10–60 days
                </div>
              </div>
            </div>
          </div>
